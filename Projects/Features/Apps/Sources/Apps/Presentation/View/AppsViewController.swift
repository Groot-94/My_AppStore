...
final class AppsViewController: UIViewController {
    ...
    init(viewModel: AppsViewModel, imageLoader: ImageLoading) {
    ...
    private let featuredSectionTitle = "추천"
    private let topFreeTitle = "인기 무료 앱"
    private let topPaidTitle = "인기 유료 앱"
    ...
    private func render(_ state: AppsViewModel.State) {
        switch state {
        case .loading:
    ...
    private func rebuildSections(with feed: AppsFeed) {
    ...
    private func makeChartRow(_ item: ChartItem) -> UIView {
        ...
        tap.appID = item.id
        // 받기 버튼(GetButton)은 자체 터치를 처리하도록 셀 탭 제스처를 양보.
        tap.delegate = self
...
extension AppsViewController: UIGestureRecognizerDelegate {
